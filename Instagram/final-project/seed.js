...
async function createUser(username, email, password = "123") {
  try {
    ...
    fs.copyFileSync(
      `${__dirname}/seeds/profiles/${image}`,
      `${__dirname}/data/user/${newName}`
    );
...
async function createArticle(username, postId) {
  try {
    ...
    const userPhotos = imgs.filter((img) =>
      img.match(new RegExp("^" + username + postId))
    );
    ...
    const photos = userPhotos.map((photo) => {
      ...
      fs.copyFileSync(
        `${__dirname}/seeds/${username}/${photo}`,
        `${__dirname}/data/posts/${newName}`
      );
