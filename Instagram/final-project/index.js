...
// # 데이터베이스 연결
mongoose.connect(
  "mongodb://127.0.0.1:27017/final", // url
...
app.post("/user/login", async (req, res, next) => {
  try {
    ...
    const hashedPassword = crypto
      .pbkdf2Sync(req.body.password, user.salt, 310000, 32, "sha256")
      .toString("hex");
  ...
  try {
    ...
    if (newUser.email !== undefined) {
      if (newUser.email === "") {
        ...
        newUser.email.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/) === null
      ) {
...
app.post("/users", async (req, res, next) => {
  try {
    ...
    // 암호화된 비밀번호
    const hashedPassword = crypto
      .pbkdf2Sync(req.body.password, salt, 310000, 32, "sha256")
      .toString("hex");
  ...
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      try {
        ...
        const photos =
          files.photos instanceof Array
            ? files.photos
            : new Array(files.photos);
...
app.get("/articles", async (req, res, next) => {
  try {
    ...
    if (req.query.username) {
      ...
      const articles = await Article.find({ author: user._id }).populate(
        "author"
      );
...
app.get("/articles/:postId", async (req, res, next) => {
  try {
    ...
    const article = await Article.findById(req.params.postId).populate(
      "author"
    );
    res.json(article);
  } catch (error) {
    // await Promise에서 발생한 에러를 잡는다
    next(error);
  }
});
...
app.get("/articles/:postId/more", async (req, res, next) => {
  try {
    ...
    const prevArticle = await Article.findOne({
      ...
      created: { $lt: article.created },
    }).sort({ created: -1 });

    const nextArticle = await Article.findOne({
      author: article.author,
      created: { $gt: article.created },
    });
...
app.get("/feed", auth, async (req, res, next) => {
  try {
    ...
    const followingIds = followingList.map((following) =>
...
app.get("/profiles/:username/followerList", auth, async (req, res, next) => {
  try {
    ...
    const followerList = await Follow.find({ followingId: user._id }).populate(
      "followerId"
    );
...
app.get("/profiles/:username/followingList", auth, async (req, res, next) => {
  try {
    const user = await User.findOne({ username: req.params.username });
    const followingList = await Follow.find({ followerId: user._id }).populate(
      "followingId"
    );
